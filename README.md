...
> ✅ **第 12 步把"取原文"这一项压掉了 146×**（1896 ms → 21 ms），详见下文
> 「取原文：把占端到端 40% 的一项压掉 146×」。
> ⚠️ 因此**上面这些延迟数字是"侧车之前"的**；跟新数字对比时先确认 `--fetch-store` 用的是哪个实现。

**回查原文的第一次优化**：parquet 的 4 个分片是**按行序连续切分**的
（829,100 / 829,099 / 829,098 / 829,098，区间首尾相接），
于是改成按**行号**定位分片（`RowTextFetcher`），
不必再拿随机哈希的 `chunk_id` 去把 4 个分片全扫一遍。
**但它只解决了"去哪个分片找"** —— 进了分片还是要全扫，所以仍要 1.7 s。
第二次优化（第 12 步）才真正把这一层打掉，见下。
...
### 取原文：把占端到端 40% 的一项压掉 146×（第 12 步）

**问题不在检索、也不在模型，而在"把原文捞出来"这一行代码。**

| | 改前 | 改后 | 倍数 |
|---|---|---|---|
| 取原文（微基准，取 100 条） | 1716 ms | **0.9 ~ 14.6 ms** | 114 ~ 1900× |
| 取原文（链路内实测，25 题中位） | 3069 ms | **21 ms** | **146×** |
| 检索链路合计（同批 25 题） | 6096 ms | **3150 ms** | 1.9× |
| 端到端（云端生产配置，**外推**） | 4.78 s | **约 2.9 s** | — |

**根因：parquet 是为"扫全表"设计的，不是为"点查"设计的。**

1. 取一行要先定位 row group，再把**整列解压**出来。实测：4 分片 × 552 MB，
   每分片 5 个 row group（20.2 万行 / 179 MB 未压缩），压缩比仅 1.3×。
2. 过滤条件用的是 `chunk_id.isin(...)`，而 chunk_id 是**随机哈希** ——
   行组的 min/max 统计覆盖全域，**剪枝率为 0**，4 个分片全部打开。
3. 于是出现反直觉的结果：**取 5 行和取 100 行一样慢**（都要先解压整片）。
   ⇒ 任何"少取几条"的思路都是无效优化，必须换存储布局。

**做法：偏移数组 + mmap 的派生存储（侧车）**

```
data/index/fetch_store/chunks_text.jsonl     331.6 万行，每行一条 JSON（id/title/section/text）
data/index/fetch_store/chunks_text.off.npy   int64 偏移数组（N+1）
```

取第 i 行 = `mmap[off[i]:off[i+1]]` —— O(1)，**不解压、不扫描，读的字节数与库大小无关**。
取 100 行约 87 KB，而全扫一次是 2060 MB。

> ⚠️ **体积没有变小**（2.87 GB，和 parquet 的 2.21 GB 同量级），**变的是布局**。
> 同样的字节数，"按行放"和"按列 + 行组放"的取数代价差两个数量级。

**四层一致性防护**（派生数据不校验就等于埋雷）：
原子替换（先写 `.tmp` 再改名）→ 行数契约（侧车行数 == ids.txt 行数）
→ 抽样逐字段自检（`--verify 300`，比 4 个字段而不只是 chunk_id）
→ 每次取回错位断言（`payload["id"] == ids[row]`，成本几微秒）。
**残留风险必须"响"而不是静默错位** —— 这是底线。

**方法论三条（比这次的加速比更值钱）：**

1. **先诊断，且诊断可以不花钱**：`diagnose_fetch.py` 不加载 faiss / BM25 / 模型，
   1 分钟量出分项延迟 —— 因为取原文的耗时只取决于"要哪些行号"和"存储布局"，
   两者都不需要索引。
2. **归因别用墙钟**：本机实测出现过"冷盘 907 ms < 热盘 1822 ms"（后台进程干扰）。
   改用 **CPU 时间 + 磁盘读字节**：`CPU 5578 ms（墙钟 325%，即并行解压吃 3.3 个核）+ 磁盘读 2060 MB`。
3. **benchmark 的工作量构造错了，结论就是错的**：诊断第一版把"真实命中"按行号排序后
   切连续段 → 那 100 行恰好落在**同一分片** → 量出来快近一倍、冷热差为负。
   改成随机抽样之后两边才可比。

**顺带发现（观察项，未当结论）**：换掉全扫后**向量检索也快了 2.3×**（542 → 232 ms）——
推测是旧实现并行解压抢 CPU、并把 2.2 GB 分片塞进页缓存挤压了其他环节。
两次运行有先后顺序，存在混杂因素，标为待验证。

📄 完整过程与坑清单：[`第12步-任务清单.md`](第12步-任务清单.md) ｜
**照着做的一页版**：[`第12步-操作步骤-简化版.md`](第12步-操作步骤-简化版.md)
...
│   ├── evaluate_gen.py        # ⑪ 生成侧四类指标（拒答/误拒/引用/FA，**离线秒级，不碰索引**）
...
│   │   ├── fetch_store/       # ⑫ 取原文侧车（2.87 GB JSONL + 26.5 MB 偏移）—— 派生数据，可重建
...
├── 第12步-任务清单.md          # ⑫ 取原文延迟优化：诊断 → 方案 → 验收 → 坑
├── 第12步-操作步骤-简化版.md   # ⑫ 一页版：4 步命令 + 8 个注意点 + 6 个知识点
