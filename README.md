...
### 双路 LLM 对照（M4 的一个消融维度，先给出定性观察）

| | 云端 qwen-plus | 本地 Qwen2.5-1.5B-Instruct 4bit |
|---|---|---|
| 拒答 | 稳定 | ✅ 稳定（实测「根据已有资料无法回答」） |
| 答案质量 | 好，会主动区分相似实体 | 能答对要点 |
| **引用标注** | ✅ 稳定标注 `[n]` | ❌ **内容对但不标编号** |
| 成本 / 显存 | API 计费 | 0 / 1.07 GB |

本地路的差距不在"答得对不对"，而在**指令遵守率**（不标引用）——
这正是"生成模型"这个消融维度要量化的东西。
...
| **1 条题答案不完整** | 1/129 | `v1-d27c79` 问"宽度和厚度各是多少"，答案只给了 `2.6毫米`。**检索指标不受影响**（只看 chunk_id），但**生成侧评测前必须补全** |
